...
--
-- Problem: user_favorite_communities, user_pinned_message, and
-- user_accent_unlocks all have USING (true) / WITH CHECK (true) policies and
...
--   3. SELECT remains open (reads are not sensitive; favorites/pins are
...
REVOKE INSERT, UPDATE, DELETE ON public.user_pinned_message FROM anon;
...
CREATE POLICY "user_pinned_message_insert_own"
  ON public.user_pinned_message
  FOR INSERT
  TO authenticated
  WITH CHECK (user_id = public.current_user_id()::text);

CREATE POLICY "user_pinned_message_update_own"
  ON public.user_pinned_message
  FOR UPDATE
  TO authenticated
  USING  (user_id = public.current_user_id()::text)
...
CREATE POLICY "user_pinned_message_delete_own"
  ON public.user_pinned_message
  FOR DELETE
  TO authenticated
  USING (user_id = public.current_user_id()::text);
