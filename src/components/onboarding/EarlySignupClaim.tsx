...
export function EarlySignupClaim({ userId, onResolved }: EarlySignupClaimProps) {
  ...
  // Server-side eligibility check on mount. If ineligible OR the RPC
  // errors for any reason, skip this step entirely so onboarding can
  // never get stuck on the reward page.
  useEffect(() => {
    let cancelled = false;
    void (async () => {
      let eligible = false;
