  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        // Ignore audio init errors in unsupported environments.
      }
    };
    init();
    return () => {
...
export function Terminal({
  ...
  return (
    <div
      ref={containerRef}
      className={cn(
        "mx-auto w-full max-w-xl px-4 font-mono text-sm sm:text-[0.7875rem]",
        className,
      )}
    >
      <div className="overflow-hidden rounded-2xl border border-raw-border bg-raw-black shadow-2xl">
        ...
        <div
          ref={contentRef}
          className="no-visible-scrollbar h-96 overflow-y-auto p-4 font-mono sm:h-80"
        >
          {lines.map((line, i) => (
