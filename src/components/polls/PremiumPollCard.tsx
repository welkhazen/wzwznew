...
const BUTTON_CLIP =
  "polygon(10% 0, 90% 0, 100% 22%, 100% 78%, 90% 100%, 10% 100%, 0 78%, 0 22%)";
const SWIPE_THRESHOLD = 78;
const VELOCITY_THRESHOLD = 520;

function getPercent(optionVotes: number, totalVotes: number, isSelected: boolean) {
  if (totalVotes <= 0) return isSelected ? 100 : 0;
...
function EdgeDetails() {
  return (
    <div className="pointer-events-none absolute inset-0">
      <span className="absolute left-4 top-3 h-px w-8 bg-raw-gold/45" />
      <span className="absolute right-4 top-3 h-px w-8 bg-raw-silver/20" />
      <span className="absolute bottom-3 left-4 h-px w-8 bg-raw-silver/15" />
      <span className="absolute bottom-3 right-4 h-px w-8 bg-raw-gold/35" />
      <span className="absolute left-1 top-12 h-20 w-px bg-gradient-to-b from-transparent via-raw-gold/45 to-transparent" />
      <span className="absolute right-1 top-12 h-20 w-px bg-gradient-to-b from-transparent via-raw-silver/25 to-transparent" />
      <span className="absolute inset-x-10 top-0 h-px bg-gradient-to-r from-transparent via-raw-gold/65 to-transparent" />
      <span className="absolute inset-x-10 bottom-0 h-px bg-gradient-to-r from-transparent via-raw-gold/35 to-transparent" />
    </div>
  );
}
...
export function PremiumPollCard({
  ...
  const handleDragEnd = useCallback(
    (_event: MouseEvent | TouchEvent | PointerEvent, info: { offset: { x: number }; velocity: { x: number } }) => {
      ...
      const shouldVotePrimary = info.offset.x > SWIPE_THRESHOLD || info.velocity.x > VELOCITY_THRESHOLD;
      const shouldVoteSecondary = info.offset.x < -SWIPE_THRESHOLD || info.velocity.x < -VELOCITY_THRESHOLD;

      if (shouldVotePrimary) {
        const animation = reduceMotion
          ? Promise.resolve()
          : animate(x, 520, { type: "spring", stiffness: 180, damping: 24 });
        Promise.resolve(animation).then(() => submitVote(primaryOption.id));
        return;
      }

      if (shouldVoteSecondary) {
        const animation = reduceMotion
          ? Promise.resolve()
          : animate(x, -520, { type: "spring", stiffness: 180, damping: 24 });
        Promise.resolve(animation).then(() => submitVote(secondaryOption.id));
        return;
      }

      animate(x, 0, { type: "spring", stiffness: 420, damping: 32 });
    },
    [disabled, isAnswered, primaryOption.id, reduceMotion, secondaryOption.id, submitVote, x]
  );
            ...
            <div className="mt-auto w-full pt-5">
              <div className="grid grid-cols-2 gap-3">
                <button
                  ...
                  className={cn(
                    "group relative min-h-[4.8rem] overflow-hidden px-3 py-3 text-center font-display text-lg tracking-wide transition duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-raw-gold/80 disabled:cursor-not-allowed",
