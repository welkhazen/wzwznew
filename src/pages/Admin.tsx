...
function AdminSignIn({ onLogin }: { onLogin: (username: string, password: string) => Promise<AuthResult> }) {
  ...
  return (
    <div className="min-h-screen bg-raw-black px-6 py-10 text-raw-text">
      <div className="mx-auto flex min-h-[calc(100vh-5rem)] max-w-md items-center">
        <div className="w-full rounded-3xl border border-raw-border/30 bg-raw-surface/25 p-6 shadow-2xl">
          <div className="text-center">
            <p className="text-xs uppercase tracking-[0.28em] text-raw-gold/65">Admin page</p>
            <h1 className="mt-4 font-display text-3xl tracking-wide">Admin sign in</h1>
            <p className="mt-3 text-sm text-raw-silver/45">
              Sign in with an admin account to manage polls, reports, users, and blocked words.
            </p>
          </div>
          ...
          <form onSubmit={handleLogin} className="mt-8 space-y-4">
            ...
            >
              {isSubmitting ? "Checking..." : "Sign in as admin"}
            </button>
          </form>

          <Link
            to="/"
            className="mt-5 inline-flex w-full justify-center rounded-xl border border-raw-border/30 px-4 py-3 text-sm text-raw-silver/60 transition-colors hover:text-raw-text"
          >
            Back to app
          </Link>
        </div>
...
export default function Admin() {
  const { user, isLoggedIn, isAdmin, login, logout } = useRawStore();
  ...
  if (!isLoggedIn || !user) {
    return <AdminSignIn onLogin={login} />;
  }
